...
public class BookSelectionListener implements ISelectionChangedListener {
    ...
    private Shell chapterNumbersPopupShell;
    private boolean dontRunEventHandler;

    @Override
    public void selectionChanged(SelectionChangedEvent event) {
        closeChapterNumbersPopupShell();
        if (dontRunEventHandler) {
            dontRunEventHandler = false;
            return;
        }
        Object selectedElement = getSelectedElement(event);
        if (selectedElement instanceof Book) {
            showNewChapterNumbersShell(event, (Book) selectedElement);
        }
    }

    private void closeChapterNumbersPopupShell() {
        if (chapterNumbersPopupShell != null && !chapterNumbersPopupShell.isDisposed()) {
            chapterNumbersPopupShell.close();
            chapterNumbersPopupShell = null;
        }
    }

    private void closeChapterNumbersShell(int delay) {
    ...
    private void showNewChapterNumbersShell(SelectionChangedEvent event, final Book selectedBook) {
        final Display display = Display.getDefault();
        createPopupShell(display);
        Composite comp = createMainComposite();
        showPopup(event, selectedBook, display, comp);
        addListeners(comp);
    }

    private void createPopupShell(final Display display) {
        chapterNumbersPopupShell = new Shell(display, SWT.ON_TOP | SWT.FOCUSED);
        chapterNumbersPopupShell.setLayout(new FillLayout());
    }

    private Composite createMainComposite() {
        Composite comp = new Composite(chapterNumbersPopupShell, SWT.None);
        comp.setLayout(new RowLayout(SWT.HORIZONTAL));
        return comp;
    }

    private void showPopup(SelectionChangedEvent event, final Book selectedBook, final Display display,
            Composite comp) {
        int numOfChapters = getNumberOfChapters(selectedBook);
        int height = calculateHeight(numOfChapters);
        chapterNumbersPopupShell.setSize(100, height);
        chapterNumbersPopupShell.setLocation(calculateLocation(display, getTree(event), height));
        createLabels(selectedBook, numOfChapters, comp);
        comp.layout();
        chapterNumbersPopupShell.open();
    }
        ...
        label.addMouseListener(new MouseAdapter() {
            ...
            public void mouseDown(MouseEvent e) {
                ...
                if (e.data == null) {
                    ...
                    closeChapterNumbersShell((int) e.data);
                }
            }
        });
    }
